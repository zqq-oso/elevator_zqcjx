...
class PassengerLog:
    ...
    def appear(self, pid: int, tick: int, floor: int, dest: int, elevators: List[ProxyElevator]) -> None:
        ...
        self.records[pid] = {
            ...
            "alight_tick": "",
            "miss_reason": "",
            "waiting_passengers": waiting_str,  # ← 新增列
            "elevator_snapshot": snapshot,      # ← 新增列
        }
    ...
    def save_csv(self, file: Path) -> None:
        ...
        with file.with_name(file_name).open("w", newline="", encoding="utf-8-sig") as f:
            writer = csv.DictWriter(f, fieldnames=[
                "passenger_id", "from_floor", "to_floor",
                "appear_tick", "board_tick", "alight_tick", "miss_reason",
                "waiting_passengers", "elevator_snapshot"  # ← 新增两列
            ])
            writer.writeheader()
            writer.writerows(self.records.values())
        print(f"[CSV] 已保存 {file.with_name(file_name).absolute()}  （文件名含场景：{self.scenario_name}）")


# ---------- 控制器 ----------
class Controller(ElevatorController):
    def __init__(self):
        super().__init__("http://127.0.0.1:8000", debug=False)
        self.max_floor = 0
    ...
    def on_init(self, elevators: List[ProxyElevator], floors: List[ProxyFloor]) -> None:
        ...
        # ===== 预读 traffic 文件拿场景 =====
        from pathlib import Path
        traffic_dir = Path(__file__).parent / "elevator_saga" / "traffic"
